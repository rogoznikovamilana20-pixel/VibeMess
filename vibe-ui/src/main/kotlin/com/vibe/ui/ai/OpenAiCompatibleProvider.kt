...
class OpenAiCompatibleProvider(
    ...
    override val name: String = "OpenAI-compatible ($baseUrl)"
    override val isAvailable: Boolean get() = baseUrl.isNotBlank()

    override fun getDefaultModel(): String = model

    override fun getAvailableModels(): List<String> = listOf(model)
    ...
    override suspend fun chat(
        ...
        try {
            connection.apply {
                ...
                setRequestProperty("HTTP-Referer", "https://github.com/vibemess")
                setRequestProperty("X-Title", "VibeMess")
                doOutput = true
            ...
            if (responseCode !in 200..299) {
                ...
                val msg = when (responseCode) {
                    402 -> "Недостаточно средств. Пополните баланс на platform.deepseek.com"
        ...
        try {
            ...
            while (true) {
                ...
                try {
                    ...
                    VibeLogger.w("OpenAiStream", "parse error: $data", e)
                }
            }
            ...
            throw AiException.Network("Ошибка соединения: ${e.message}")
        } finally {
            connection.disconnect()
        }
